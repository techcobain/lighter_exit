...
import { POOL_STATUS, TX_TYPE } from '../../lib/config'
import { formatAmount, formatUsd } from '../../lib/format'
import { BTN_PRIMARY_CLASSNAME, BTN_SECONDARY_CLASSNAME, LABEL_CLASSNAME } from '../../lib/recipes'
import { signBurnShares, signUpdatePublicPool } from '../../lib/signer'
...
export function PoolWindDownStep({
  ...
  const stepState: StepState = !info
    ...
    : operatorShares === 0n && frozen
      ? 'done'
      : isInFlight(freezeState) || isInFlight(burnState)
        ? 'active'
        : 'todo'
      ...
      {!info ? (
        ...
        <div className="flex flex-col gap-5">
          <div className="grid grid-cols-2 gap-4 max-mobile:grid-cols-1">
            ...
            <Stat label="Pool value" value={formatUsd(pool.total_asset_value)} />
            <Stat label="Your operator shares" value={formatAmount(operatorShares.toString(), 0)} />
            <Stat label="Total shares" value={formatAmount(String(info.total_shares), 0)} />
          </div>
